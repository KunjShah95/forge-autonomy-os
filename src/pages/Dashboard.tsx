    ...
    <div className="space-y-6">
      ...
      <div className="relative glass rounded-2xl p-6 overflow-hidden">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="mono text-[10px] uppercase tracking-[0.2em] text-primary mb-2 flex items-center gap-2">
          ...
          <div className="flex gap-2">
            <button className="mono text-xs px-3 py-2 rounded-md border border-border hover:border-primary/40 transition flex items-center gap-2"><Zap className="h-3.5 w-3.5" /> Trigger run</button>
            <button className="mono text-xs px-3 py-2 rounded-md bg-gradient-to-r from-primary to-accent text-primary-foreground shadow-[0_0_20px_hsl(var(--primary)/0.4)] flex items-center gap-2"><Rocket className="h-3.5 w-3.5" /> Deploy v2.14.3</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Panel eyebrow="Autonomous" title="Decision Feed" className="h-[360px]">
          <div className="p-4 space-y-3 text-sm overflow-y-auto h-full">
            {[
              { who: "SRE Agent", what: "Auto-rollback of ml-inference v0.9.2", why: "Error budget burned 4.2x in 10m window", risk: 78 },
              { who: "DevOps Agent", what: "Promote users-svc canary to 50%", why: "0 errors over 12m bake, latency stable", risk: 14 },
              { who: "Security Agent", what: "Patch CVE-2025-31021 in auth-svc", why: "Critical CVSS 9.1 in transitive dep", risk: 42 },
              { who: "QA Agent", what: "Block deploy of billing-svc v1.22.0", why: "Synthesized regression hit known canary failure", risk: 64 },
            ].map((d, i) => (
              <div key={i} className="p-3 rounded-lg border border-border bg-secondary/40 hover:border-primary/30 transition group">
                <div className="flex items-center justify-between mb-1">
                  <span className="mono text-[10px] text-accent uppercase tracking-widest">{d.who}</span>
                  <span className="mono text-[10px] text-muted-foreground">risk {d.risk}</span>
                </div>
                <div className="font-medium text-sm">{d.what}</div>
                <div className="text-xs text-muted-foreground mt-1">{d.why}</div>
              </div>
            ))}
          </div>
        </Panel>
